...
function onInstall() {
    onOpen();
    SpreadsheetApp.getUi().alert("Keystone Invoice", "The Keystone Invoice addon has been installed. To run the script functions," +
        "go to the Add-ons menu, and trigger the functions from the Keystone Invoice submenu.", SpreadsheetApp.getUi().ButtonSet.OK);
}
...
function onInvoice() {
    ...
    for (var i = 0; i < sheet_values.length; i++) {
        ...
        for (var j = 0; j < pairs.length; j++) {
            var new_values = sheet_values[i].map(function (old_value) {
...
function adminSubs() {
    ...
    for (var _i = 0, pairRange_1 = pairRange; _i < pairRange_1.length; _i++) {
        var pair = pairRange_1[_i];
        if (pair[0] === "" || pair[1] === "") {
